...
include $root_path . 'web-himsi/config/db.php';

if (!isset($_GET['folder_id'])) {
    echo "<script>alert('Folder ID tidak ditemukan 1.'); window.location='gallery.php';</script>";
    exit;
}
...
<body class="bg-gradient-to-br from-blue-50 to-white min-h-screen">
  <header class="px-6 py-4 flex justify-between items-center sticky top-0 z-30" style="background-color: #043464;">
    ...
    <button
      ...
      onclick="window.location.href='http://localhost/himsi/main.php'"
    >
        ...
        <?php else: ?>
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                <?php while ($photo = mysqli_fetch_assoc($photos_result)): ?>
                    <div class="border rounded overflow-hidden shadow hover:shadow-lg transition cursor-pointer" onclick="openModal('<?php echo '/himsi/asset/gallery_img/' . htmlspecialchars($photo['nama_file']); ?>')">
                        <img src="<?php echo '/himsi/asset/gallery_img/' . htmlspecialchars($photo['nama_file']); ?>" alt="<?php echo htmlspecialchars($photo['deskripsi'] ?? 'Photo'); ?>" class="w-full h-48 object-cover" />
                        <div class="p-4">
                            ...
                            <a href="<?php echo '/himsi/asset/gallery_img/' . htmlspecialchars($photo['nama_file']); ?>" download class="inline-block mt-2 px-3 py-1 bg-blue-600 text-white rounded hover:bg-blue-700 text-xs">Download</a>
                        </div>
                    </div>
                <?php endwhile; ?>
            </div>
